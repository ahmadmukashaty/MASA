angular.module('starter.services', [])
  ...
  .factory('CartService', function($ionicLoading, $rootScope, $http, appConfig, appValue) {
    ...
    return {
      ...
      getContainerTotal: function() {
        var total = 0;
        var count = 0;
        angular.forEach(cartInfo.products, function(product, key) {
          count += product[1];
          if (product[2].full_container_count !== null) {
            total = total + (1 / product[2].full_container_count) * product[1];
          }
        });
        if (total.toFixed(2) == 0.0 && count > 0) {
          return "<0.01";
        } else {
          return total.toFixed(2);
        }
      },
      getTotalCBM: function() {
        var total = 0;
        var count = 0;
        angular.forEach(cartInfo.products, function(product, key) {
          count += product[1];
          if (product[2].cbm !== null) {
            total = total + (product[2].cbm) * product[1];
          }
        });
        if (total.toFixed(3) == 0.0 && count > 0) {
          return "<0.001";
        } else {
      ...
      getWeightTotal: function() {
        ...
        angular.forEach(cartInfo.products, function(product, key) {
          count += product[1];
          if (product[2].weight_in_container !== null) {
            total = total + product[2].weight_in_container * product[1];
          }
        });
        if (total.toFixed(2) == 0.0 && count > 0) {
          return "<0.01";
  ...
  .factory('AnimationService', function($timeout, $ionicPosition) {
    return {
      ...
      moveProductToCartAnimation: function(imgElemid /* = "single-main-img"*/ ) {
        ...
        var parentElem = angular.element(document.getElementById("single-product")),
          ...
          offsetTopCart = 10,
           ...
           var imgElem = angular.element(document.getElementsByClassName(imgElemid)[0]);
           if (!imgElem){
            imgElem = angular.element(document.getElementsByClassName(imgElemid));
           }
        ...
        imgClone.css({
          ...
          //'left': (parentPosition.width/2 - 75) +'px',
          //	'left': ($ionicPosition.offset(imgElem).left + ($ionicPosition.offset(imgElem).width/2) -38) +'px',
          'opacity': 0.5
        });
  ...
  .factory('ResourceService', function($http, $ionicLoading, appConfig, appValue) {
    var listImages = [];
    return {
      getSliderImages: function() {

        var _url = appConfig.DOMAIN_URL + appValue.API_URL + 'resources?type=slider-images';
        if (typeof $url != "undefined") {
          _url += $url;
        }
        $http({
          ...
          .then(function(response) {
            ...
            // handle success things
            if (response.data.status === appValue.API_SUCCESS) {
              angular.forEach(response.data.data, function(slider, key) {
                listImages.push(slider);
              });
              return listImages;
            } else {
              //handle errors
