...
.run(function($ionicPlatform, $ionicPopup, $rootScope, AppService, appConfig, $injector, AuthService,ResourceService) {
  ...
  $rootScope.getCopyrightsString = function () {
		var year = (new Date()).getFullYear();
		return $rootScope.appLanguage.COPYRIGHTS_STRING.replace(":year", year);
	}
  $rootScope.sliderImages = ResourceService.getSliderImages();
})
...
.config(function($stateProvider, $urlRouterProvider, $ionicConfigProvider, appConfig) {
  ...
  .state('tab.checkout-success', {
    ...
    .state('tab.view-image', {
           url: '/view-image',
           views: {
           'mainContent': {
           templateUrl: 'templates/'+appConfig.ENABLE_THEME+'/view-image.html',
