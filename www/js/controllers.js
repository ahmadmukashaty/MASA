angular.module('starter.controllers', ['ionic-ratings'])
      ...
      //#region AppCtrl
      .controller('AppCtrl', function ($scope, $state, $ionicSideMenuDelegate, $location, $sce, $rootScope, $ionicModal, AppService, UserService, CartService, WishlistService, $timeout, AnimationService, appConfig, appLanguage, LanguageService, AuthService, ListCategoryService) {

            //check if the app should enforce user to log in first or not
            //by check ENABLE_FIRST_LOGIN variable
            ...
            $scope.goCartPage = function () {
                  $rootScope.hideTabs = 'tabs-item-hide'; // to not show footer tab
                  $state.go('tab.cart');
            };
            $scope.userInfo = {};
            $scope.loginData = {};
            ...
            $rootScope.addToCart = function (product) {
                  ...
                  AnimationService.moveProductToCartAnimation("img-" + productId);
                  $timeout(function () {
                        AnimationService.actionByClass('shoppingcart', 'swing');
                        $rootScope.cartQuantity = CartService.getCartQuantity();
                  }, 1000);
                  if (WishlistService.checkProductInWishlist(productId)) {
                        WishlistService.removeProductFromWishlist(productId);
                  }
                  return true;
            };
            $rootScope.isProductInWishlist = WishlistService.checkProductInWishlist;
            $rootScope.addProductToWishlist = function (product) {
                  //something to add product into cart
                  var productId = product.id;
                  var quantity = 1;
                  var info = product;
            ...
            $rootScope.CategoryMenuData = [];
            $rootScope.getCategoryMenuData = function () {
                  ListCategoryService.getListCategory()
                        .then(function (categoryList) {
                              $rootScope.CategoryMenuData = categoryList;
                        });
            };
            $rootScope.getCategoryMenuData();
            $rootScope.openMenuCategory = function ($categoryId) {
                  angular.forEach($rootScope.CategoryMenuData, function (category, key) {
      ...
      //#region HomeCtrl
      .controller('HomeCtrl', function ($scope, $state, $rootScope, ProductService, AppService, ResourceService, $ionicSlideBoxDelegate) {
            $scope.productData = [];
            $scope.pageProduct = 1;
            ...
            $scope.refreshSlider = function () {
                  $scope.sliderImages = ResourceService.getSliderImages();
                  dataChangeHandler();
            };
      ...
      //#region SingleCategoryCtrl
      .controller('SingleCategoryCtrl', function ($scope, $stateParams, $state, ProductService, AppService) {
      ...
      //#region WishlistCtrl
      .controller('WishlistCtrl', function ($scope, $rootScope, $stateParams, $state, WishlistService) {
      ...
      //#region ProductCtrl
      .controller('ProductCtrl', function ($scope, $ionicModal, $sce, $timeout, $ionicPopup, $rootScope, $ionicSlideBoxDelegate, appConfig, CartService, WishlistService, ReviewsService, UserService, AnimationService, $state, DataService) {
            $scope.product = JSON.parse(window.localStorage.getItem("product"));
            $scope.Sliding = function (groupName) {
                  $className = ".sliderToggleView_" + groupName;
                  $($className).slideToggle("slow");
            };


            $scope.novalue = "NO VALUE";
            ...
            //                  try {
            //
            //                        PhotoViewer.show(srcImage, productName, {
            //                              share: false
            //                        });
            //                  } catch (err) {
            //                        $ionicPopup.alert({
                        ...
                        if (count === $scope.productFormData.attributes.length) {
                              ...
                              if (variation.image) {
                                    ...
                                    if (!flagVariationImage) {
                                          ...
                                          $ionicSlideBoxDelegate.update();
                                          $timeout(function () {
                                                $ionicSlideBoxDelegate.slide($scope.product.images.length - 1);
                                          }, 100);
                                    }
                              }
                              if (variation.images) {
            ...
            //prepare image urls for image viewer components
            $scope.product.imagesUrls = [];
            angular.forEach($scope.product.images, function (image, key) {
                  $scope.product.imagesUrls.push(image.src);
            });
            $scope.viewZoomProductImage = function (srcImage, productName) {
                  DataService.setImageUrls($scope.product.imagesUrls);
                  $state.go('tab.view-image', {});
            };
            angular.forEach($scope.product.attributes, function (attribute, key1) {
                  if (attribute != null && attribute.variation != true) {
            ...
            $scope.tempmasaAttreibutes = angular.copy($scope.product.masaAttreibutes);
            angular.forEach($scope.tempmasaAttreibutes, function (attr, key) {
                  $scope.numberOfDeletedItem = 0;
                  angular.forEach(attr.attributes, function (attribute, key1) {
                        if (attribute.value == $scope.novalue) {
                              $scope.product.masaAttreibutes[key].attributes.splice(key1 - $scope.numberOfDeletedItem, 1);
                              $scope.numberOfDeletedItem++;
                        }
                  });
            });
            // $scope.numberOfDeletedItem = 0;
            // angular.forEach($scope.tempmasaAttreibutes, function(attribute, key1) {
            // if(attribute.value == $scope.novalue) {
            //     $scope.product.masaAttreibutes.splice(key1 - $scope.numberOfDeletedItem,1);
            //     $scope.numberOfDeletedItem++;
            // }
            // });
            ...
            $scope.addProductToCart = function () {
                  ...
                  CartService.addProductToCart(productId, quantity, info);
                  AnimationService.moveProductToCartAnimation('single-main-img');
                  $timeout(function () {
                        AnimationService.actionByClass('shoppingcart', 'swing');
                        $rootScope.cartQuantity = CartService.getCartQuantity();
                  }, 1000);
      ...
      //#endregion
      //#region ViewImageCtrl
      .controller('ViewImageCtrl', function ($scope, $rootScope, $state, /*$timeout,*/ $ionicHistory, DataService) {
            $scope.closeClicked = function () {
                  $ionicHistory.goBack();
            }
            // var elem = angular.element(window.document).find('#zoom-slider');
            // var init = new Powerange(elem, { vertical: true });
            $scope.value = 75;
            $scope.min = 10;
            $scope.max = 90;
            $scope.imageUrls = DataService.getImageUrls();
      ...
      //end
      //#region CartCtrl
      .controller('CartCtrl', function ($scope, $rootScope, $state, $timeout, UserService, CartService, AnimationService) {
            ...
            $scope.goHomePage = function () {
                  $rootScope.hideTabs = ''; // to show footer tab
                  $state.go('tab.home');
            };
            $scope.cartEmpty = false;
            $scope.cartUpdatedProduct = {};
            //$scope.totalContainerCount = 1;
            $scope.dirtyInput = false;
            $scope.refresh = function () {
                  $scope.cartInfo = CartService.getCartInfo();
                  $rootScope.cartQuantity = CartService.getCartQuantity();
                  $scope.cartTotal = CartService.getCartTotal();
                  $scope.totalContainerCount = CartService.getContainerTotal();
                  $scope.totalWeight = CartService.getWeightTotal();
                  $scope.totalCBM = CartService.getTotalCBM();
                  if ($scope.cartQuantity == 0 || isNaN($scope.cartQuantity)) {
                        $scope.productCount = $scope.cartInfo.products.length;
                        $scope.CartStatusChecker = function () {
                              // if (isNaN($scope.cartQuantity) || $scope.cartQuantity == 0) {
                              $scope.totalWeight = 0.0;
                              $scope.totalCBM = 0.0;
                              $scope.cartTotal = 0.0;
                              $scope.totalContainerCount = 0;
                              //  }
                              if ($scope.productCount != 0) // To keep price form shown also when there is no quantity or null
                                    $scope.cartEmpty = false;
                              if ($scope.cartQuantity == 0 && $scope.productCount == 0)
                                    $scope.cartEmpty = true;
                        };
                        $scope.CartStatusChecker();
            ...
            $scope.updateCart = function ($productId) {
                  // AnimationService.action('cart_item_' + $productId, 'bounceIn', 1000);
                  CartService.updateCart($productId, $scope.cartUpdatedProduct[$productId]);
                  $scope.refresh();
                  $scope.dirtyInput = true;
            };
            $scope.quantityAdd = function ($productId, delta) {
                  //  AnimationService.action('cart_item_' + $productId, 'bounceIn', 1000);
            ...
            $scope.quantityChange = function ($productId, delta) {
                  //  AnimationService.action('cart_item_' + $productId, 'bounceIn', 1000);
                  CartService.addProductToCart($productId, delta);
                  //  $rootScope.cartQuantity = CartService.getCartQuantity();
                  $scope.refresh();
            ...
            // By Mhd Obeid 
            $scope.itemQuantityChecker = function ($quantity) {
                  $rootScope.cartQuantity = CartService.getCartQuantity();
                  $scope.quantityZero = false;
                  $scope.cartEmpty = false;
                  $scope.isNan = false;
                  if ($quantity == null)
                        $scope.quantityZero = true;
                  if ($scope.cartQuantity == 0)
                        $scope.cartEmpty = true;
      ...
      //#region CheckoutBillingCtrl
      .controller('CheckoutBillingCtrl', function ($scope, $state, $ionicPopup, $rootScope, AppService, UserService, OrderService) {
            $scope.userInfo = UserService.getUser();
            if ($scope.userInfo.billing.state == '') {
                  $scope.userInfo.billing.state = 'no state';
            }
            if ($scope.userInfo.billing.postcode == '') {
                  $scope.userInfo.billing.postcode = 'no zipcode';
            }
            if ($scope.userInfo.billing.email == '') {
                  $scope.userInfo.billing.email = $scope.userInfo.email;
            }
            ...
            $scope.changeCountry = function () {
                  ...
                  $scope.userInfo.shipping.state = 'no state';
            };
      ...
      .controller('CheckoutNoteCtrl', function ($scope, $state, $ionicPopup, $rootScope, AppService, UserService, OrderService) {
            ...
            if ($scope.userInfo.shipping.state == '') {
                  $scope.userInfo.shipping.state = 'no state';
            }
            if ($scope.userInfo.shipping.state == '') {
                  $scope.userInfo.shipping.state = 'no zipcode';
            }
            $scope.listCountry = {};
      ...
      .controller('CheckoutPaymentCtrl', function ($scope, $state, $ionicHistory, $ionicPopup, $rootScope, OrderService, UserService, CartService, appConfig, $injector, appValue) {
            ...
            $scope.cartTotalWeight = CartService.getWeightTotal();
            $scope.cartContainerCount = CartService.getContainerTotal();
            ...
            OrderService.createServeCart($scope.orderInfo.coupon, $scope.cartInfo.products, $scope.orderShipping.country, $scope.orderShipping.state, $scope.orderShipping.postcode).then(function (result) {
                  ...
                  $scope.cartTotal = /* $scope.cartSubTotal + */ parseFloat($scope.shippingCost) + OrderService.getOrderGrandTotal();
                  var firstPayment = '';
                  ...
                  angular.forEach($scope.listShipping, function (shipping, key) {
                        if (shipping.id === $scope.orderInfo.shipping) {
                              if (shipping.cost !== undefined && shipping.cost !== "0" && shipping.cost !== "") {
                                    ...
                                    $scope.shippingCost = 0;
                                    $scope.cartTotal = /*$scope.cartSubTotal + */ OrderService.getOrderGrandTotal();
            ...
            $scope.createOrder = function () {
                  ...
                  if (appConfig.ENABLE_STRIPE_PLUGIN === true && $scope.orderInfo.payment === 'stripe' && appValue.API_URL == '/module/icymobi/') {
                        try {
                              StripeService = $injector.get('StripePrestashopService');
                              StripeService.init();
                        } catch (err) {
                              $ionicPopup.alert({
                                    title: $rootScope.appLanguage.MESSAGE_TEXT,
            ...
            $scope.paymentDataRefresh = function () {    // By mhd obeid
                  $scope.cartInfo = CartService.getCartInfo();
                  $scope.cartSubTotal = CartService.getCartTotal();
                  $scope.cartTotalWeight = CartService.getWeightTotal();
                  $scope.cartContainerCount = CartService.getContainerTotal();
                  ...
                  cordova.plugins.email.isAvailable(
                        function (isAvailable) {
                              cordova.plugins.email.open({
                                    ...
                                    bcc: $scope.appSettings.contact_email_bcc,
                                    subject: $scope.appSettings.contact_email_subject,
                              });
                        }
                  );
            };
      ...
      .controller('OrdersCtrl', function ($scope, $state, UserService, OrderService) {
            ...
            $scope.getListOrderData = function ($page) {
                  OrderService.getListOrder(UserService.getUserId(), $page).then(function (listOrder) {
                        ...
                        if (listOrder.length > 0) {
                              ...
                              $scope.$broadcast('scroll.infiniteScrollComplete');
                              $scope.$broadcast('scroll.refreshComplete');
                              if (listOrder.length == 10) $scope.canLoadMoreOrderData = true;
                              else {
                                    $scope.canLoadMoreOrderData = false;
                              }
                        }
            ...
            $scope.loadMoreListOrderData($scope.pageOrder);
      })

      //#endregion

      //#region NotificationCtrl
